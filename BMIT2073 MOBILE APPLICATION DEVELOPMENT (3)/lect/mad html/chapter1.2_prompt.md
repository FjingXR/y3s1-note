Create a single interactive HTML file at `lect/mad html/chapter1_2_interactive.html` that turns Chapter 1.2 of the BMIT2073 lecture notes into a study tool. Use the same dark theme as `chapter1_intro.html` (read it first for exact CSS variables, fonts, class names, and layout patterns).
...
- **All answers must come from the lecture PDF source material** — read `Chapter 1.2 Introduction to Dart.pdf` to extract exact content
